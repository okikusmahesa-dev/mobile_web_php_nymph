	...
	$.ajax({
		...
		success: function(jsRet){
			...
			if (jsRet.status != 1) {
                ...
                var lineOne = rows.map(function(e){
                    return "<tr><td class='cells1' style='padding:5px;'>" + e.cell[2] + "</td><td class='cells2' style='padding:5px;'>" + e.cell[3] + "</td></tr>";
                }).toString()
               
                $("#priceTxt").keyup(function(){
                    var lineTwo = lineOne.replace(",","")
                    $("#ListPrice").css("display","block").css("position","absolute").css("background-color","white").css("z-index","1").css("padding","5px")
                    $("#ListPrice").html(lineTwo);
                    
                    $(".cells1").click(function(){
                        var i = $(this).text()
                        $("#priceTxt").val(i)
                        $("#ListPrice").css("display","none")
                    })

                    $(".cells2").click(function(){
                        var j = $(this).text()
                        $("#priceTxt").val(j)
                        $("#ListPrice").css("display","none")
                    })
