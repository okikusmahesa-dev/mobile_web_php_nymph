...
<head> 
	...
	<script type="text/javascript" src="js/trLibs.js?20200211b"></script>
			...
			success: function(jsRet){
				if (jsRet.status != 1){
					...
					if (security_type.code == 'A') {
						alert('Stock: ' + code + ' is Acceleration Stock');
						window.location.href="buyOrderAcExt.php";
						return;
					}
...
*/
	...
	function tr189000(id, code, board, price, lot) {
        pHashOrg = "tr=189000&userId=" + id + "&dealerId=" + id + "&pin=&code=" + code + "&mkt=" + board + "&price=" + price + "&qty=" + lot + "&lot=100" + "&type=0",
        pHash    = "phash=" + Base64.encode(pHashOrg);
    ...
    function tr800011(id, code, price, lot, oldOrdNo, pin, cash) {
        pHashOrg = "tr=800011&userId=" + id + "&code=" + code + "&price=" + price + "&lot=" + lot + "&oldOrdNo=" + oldOrdNo + "&pin=" + pin;
        pHash    = "phash=" + Base64.encode(pHashOrg);
    ...
    function tr100006(id, code, price) {
        pHashOrg = "tr=100006&userId=" + id + "&code=" + code + "&price=" + price;
        pHash    = "phash=" + Base64.encode(pHashOrg);
...
<body>
	...
	<table width=320px border=0>
		<tr>
			<td colspan="3"><h2 style="margin-bottom: 0px;">Buy Order</h2>
			<input id="commissionTxt" type="hidden" class="no-ime" data-mini="true">
			</td>
			...
			<td width="45%">
                <table border=0>
                	<tr>
                		<td>Type</td>
                		<td>
                			<select name="tipeorderlist" onChange="combo(this, 'theinput')">
								<option>Day</option>
								<option>Session</option>
								<option>Market Day</option>
								<option>Market Ses</option>
								<option>Market FAK</option>
								<option>Market FOK</option>
							</select> 
                		</td>
	                	</td>
	                </tr>
