...
$userId = $_SESSION["userId"];      // for display only
$pinLogin = 0;
...
<head> 
	...
	<script type="text/javascript">
		function tr182600(id, bsFlag, status, cnt) {
			...
			$.mobile.loading('show')
            //
            pHashOrg = sprintf("tr=182600&userId=%s&cliId=%s&pin=&code=&board=&bsFlag=%s&status=%s&from=%s&to=%s&cnt=%d&keyDate=&keyOrderNo=999999999&price=",id, id, bsFlag, status,today,today, cnt);
		...
		function withdrawFunc(celDiv, id) {
			$(celDiv).click(function() {
                ...
                var userId = "<?= $_SESSION["userId"];?>";      // for display only
...
//
		...
		function tr100000(code) {
            pHashOrg = "tr=100000&code=" + code + "&board=RG";
            pHash    = "phash=" + Base64.encode(pHashOrg);
			$.ajax({
				type: "POST",
				url: "json/trproc.php",
...
*/
		...
		function tr189300(id, oldOrd, code, price, qty) {
            pHashOrg = sprintf("tr=189300&userId=%s&dealerId=%s&pin=&oldOrdNo=%s&code=%s&board=RG&price=%s&lot=100&qty=%s", id, id, oldOrd, code, price, qty);
            pHash    = "phash=" + Base64.encode(pHashOrg);
			$.ajax({
				type: "POST",
				url: "json/trproc.php",
		...
		function tr189200(id, oldOrd, code, price, qty) {
            pHashOrg = sprintf("tr=189200&userId=%s&dealerId=%s&pin=&oldOrdNo=%s&code=%s&board=RG&price=%s&lot=100&qty=%s", id, id, oldOrd, code, price, qty);
            pHash    = "phash=" + Base64.encode(pHashOrg);
			$.ajax({
				type: "POST",
				url: "json/trproc.php",
...
<div id="withdrawDialog" data-role="page">
	...
	<div data-role="main" class="ui-content" style="background-color:#c4e2e2">
		<table border=0 width=100%>
			<tr>
				<td width="25%" style="color:#000000">User</td><td id="userIdW" width="25%" style="color:#000000" align="right"></td>
			</tr>
			<tr>
...
<div id="amendDialog" data-role="page">
	...
	<div data-role="main" class="ui-content" style="background-color:#ffe1a0">
		<table border=0 width=100%>
			<tr>
				<td width="25%" style="color:#000000">User</td><td width=25%><input id="userIdA" type="text" disabled="disabled" /></td>
			</tr>
			<tr>
