...
if ($res["out"][0]["status"] == 1) {
	...
	//print("Success.\n");
	$_SESSION["isLogin"] = 1;
	$_SESSION["loginId"] = strtoupper($_POST["loginId"]);
	$_SESSION["clientId"] = strtoupper($res["out"][0]["userId"]);
	$_SESSION["userId"] = strtoupper($res["out"][0]["userId"]);
	$_SESSION["isWork"] = $res["out"][0]["isWork"];
		...
		case "complete" :
			if ($isInput == TRUE) {
				if ($isRecord == TRUE) {
					...
					$key = $attr["bindname"];
                    if ($key == "loginId") {
                        $inMap[$key] = strtoupper($inMap[$key]);
                    } 
					$inData .= $inMap[$key] . "|";
				}
			}
...
function getTrOut($trNo, $trIn) {
	...
	while (1) {
		if (false == ($out_len = socket_read($socket, 4))) {
            error_log("socket_read() failed: reason: "
		...
		//print_r($data_len);
		//echo "len=" . $data_len[1];
        if (false == ($data_out = socket_read ($socket, $data_len[1]))) {
            error_log("socket_read() failed: reason: "
                . socket_strerror(socket_last_error($socket))) ;
            break;
        }
