...
$userId   = $_SESSION["userId"];              /// dont use
$clientId = $_SESSION["loginId"];           /// dont use
// new API
/*
20210622:
    loginId  : login id for session
    clientId : for query
    userId   : same as clientId, but dont use anymore
    dealerId : dealer id for query
*/
$userId   = "";           // current login id
$loginId  = "";           // current login id
$clientId = "";          // client id for query
...
<head> 
	...
	<script type="text/javascript">
		...
		var g_cell_v1 = [];
		var g_cash = 0.0;
		var g_totalBuyVal = 0.0;
		var g_totalMktVal = 0.0;
		var g_unrealized = 0.0;
		var g_totalStock = 0.0;
		var g_totalMarketVal = 0.0;
		var g_totalOpenBuyAmt = 0.0;
		var g_totalNet = 0.0;
		...
		$(document).ready(function() {			
			...
			hport = {
				...
				colModel: [
					...
					{display: "Cash Avlb", width:85, sortable: false, align: "center"},
					{display: "Buy Limit", width:85, sortable: false, align: "center"},
			...
			hport1 = {
				...
				height: "auto", 
				colModel :[
					{display: "T.Buy Val", width:82, sortable: false, align: "center"},
					{display: "T.Mkt Val", width:90, sortable: false, align: "center"},
					{display: "Unrealized", width:85, sortable: false, align: "center"},
					{display: "Stk Val", width:85, sortable: false, align: "center"},
					...
					{display: "Net Asset", width:85, sortable: false, align: "center"}
			...
			args = {
				...
				height : "auto",
				colModel : [
					{display:"Code", width:60, sortable:false, align:'center', process: codeFunc},
					{display:"Lot", width:40, sortable:false, align:'right'},
					{display:"Shares", width:50, sortable:false, align:'right'},
					{display:"Avg Buy", width:60, sortable:false, align:'right'},
					{display:"Last.Prc", width:60, sortable:false, align:'right'},
					//{display:"Buy Val", width:100, sortable:false, align:'right', hide:'true'},
					{display:"Avg Buy Val", width:90, sortable:false, align:'right'},
					{display:"Market Val", width:90, sortable:false, align:'right'},
					{display:"Hc", width:30, sortable:false, align:'right'},
					{display:"Stk Val", width:90, sortable:false, align:'right'},
					{display:"Unrealized", width:80, sortable:false, align:'right'},
					{display:"(%)", width:40, sortable:false, align:'right'},
					{display:"Done.B", width:50, sortable:false, align:'right'},
					{display:"Done.S", width:50, sortable:false, align:'right'},
					{display:"Open.B", width:50, sortable:false, align:'right'},
					{display:"Open.S", width:50, sortable:false, align:'right'},
					{display:"Notasi", width:30, sortable:false, align:'right', hide:'true'}
				]
			};
			...
			if (<?=$pinState;?> == 1) {
				...
				tr800300('<?=$clientId;?>', '<?=$pinLogin;?>');
				refreshAll(); 
			}
		});
		
        function codeFunc(celDiv, id) {
            $(celDiv).click(function() {
                //console.log("id=" + Number(id));
                notation = $("#row" + id + " td:nth-child(16) div").text();
                //console.log("notation=" + notation);
                //console.log(notation.trim().length);
                if (notation.trim().length > 0) {
                    alert("Notasi Khusus : " + notation);
                }
             });
        }
			...
			$.ajax({
				...
				success: function(jsRet){
					if (jsRet.status != 1) {
						...
						/*
						MarketValue = balance * lastPrice
						BuyingValue = avgPrice * balance
						balance = netBalance(db) + offer
						StockValue = BuyingValue * 
						*/
						...
						for (i = 0; i < cnt; i ++) {
							...
							*/
							...
							fLot = Number(jsRet.out[i].lot);				// Lot
							fShares = Number(jsRet.out[i].balance);			// Shares
							fAvgPrice = Number(jsRet.out[i].avgPrice);		// 0000.0000
							if (jsRet.out[i].mktPrice == "") {
								fLastPrice = 0;	
							} else {
								fLastPrice = Number(jsRet.out[i].mktPrice);		// 0000
							}
							

							fOffer = Number(jsRet.out[i].offer);
							fOfferShares = fOffer * 100;	// Shares, Later add logic to lot_size
							fBuyVal = Number(fAvgPrice * (fShares + fOfferShares)).toFixed(0);			// Calcurate or Using DB?
							//fBuyVal = Number(jsRet.out[i].stkVal);
							fMktVal = Number(fLastPrice * (fShares + fOfferShares)).toFixed(0);
							//fMktVal = Number(jsRet.out[i].mktVal);
							fHC = Number(jsRet.out[i].haircut);
							fStkValuation = Number(jsRet.out[i].lqVal).toFixed(0);
							fUnrealized = fMktVal - fBuyVal;

							fBuyValForSum = fBuyVal;
							if (stkType == "" || stkType == "B" || stkType == "D" || stkType == "O") {
								fBuyValForSum = 0.0;
								fUnrealized = 0.0;
							} 
							if (fBuyVal == 0) {
								fUnrealizedRatio = 0.00;
							} else {
								fUnrealizedRatio = fUnrealized / fBuyVal * 100.00;
							}
							if (fBuyVal == 0) {
								fUnrealized = 0.0;
							}
							if (Number(jsRet.out[i].openbuyVal) == "") {
								fOpenBuyAmt = 0;
							} else {
//								fOpenBuyAmt = Number(jsRet.out[i].openVal);
								fOpenBuyAmt = Number(jsRet.out[i].openbuyVal);
							}
							...
							console.log(sprintf("%s, fBuyVal[%s], fMktVal[%s], fUnrealized[%s], fLqVal[%s], fOpenVal[%s]", code, fBuyVal, fMktVal, fUnrealized, fStkValuation, fOpenBuyAmt ));

							// Accumulate
							totalBuyVal = totalBuyVal + Number(fBuyValForSum);
							totalMktVal = totalMktVal + Number(fMktVal);
							totalStock = totalStock + Number(fStkValuation);
							totalUnrealized = totalUnrealized + Number(fUnrealized);
							totalOpenBuyAmt = totalOpenBuyAmt + Number(fOpenBuyAmt);
							...
							if (stkType == "" || stkType == "B" || stkType == "D" || stkType == "O") {
								...
								bval = sprintf("%.2f", ((Number(shares) + (Number(ops) * 100))* Number(bprc))); //buying value
								//bval = sprintf("%d", Number(jsRet.out[i].stkVal));
							}
							mval = jsRet.out[i].mktVal;

							//totalBuyVal = totalBuyVal +Number(bval);
							//totalBuyVal = totalBuyVal +Number(bval);
							//console.log("totalBuyVal = " + totalBuyVal);
							...
							if (stkType == "" || stkType == "B" || stkType == "D" || stkType == "O") {
								...
								//unrl = sprintf("%d", Number(jsRet.out[i].mktVal) - Number(jsRet.out[i].stkVal));
								unrl = sprintf("%d", Number(jsRet.out[i].mktVal) - Number(jsRet.out[i].stkVal));
							}
							...
							*/
							...
							cell_v = [
								...
								addCommas(fLot),
								addCommas(fShares),
								addCommas(parseFloat(fAvgPrice).toFixed(2)),
								addCommas(fLastPrice),
								addCommas(fBuyVal),
								addCommas(fMktVal),
								hc,
								addCommas(parseFloat(fStkValuation).toFixed(0)),
								addCommas(parseFloat(fUnrealized).toFixed(0)),
								parseFloat(fUnrealizedRatio).toFixed(2),
								dnb,
								dns,
								opb,
								ops,
						...
						g_totalMarketVal = totalMktVal;
						g_totalBuyVal = totalBuyVal;
						g_totalMktVal = totalMktVal;
						g_unrealized = totalUnrealized;
						g_totalStock = totalStock;
						g_totalNet = totalNet;
						g_totalOpenBuyAmt = totalOpenBuyAmt;
						console.log("g_totalmarketVal = " + g_totalMarketVal);
		...
		function refreshAll() {
			...
			qryTr182600();							// refresh orderlist
			qryTr800001();							// account stock
			qryTr800011();							// cRat, pRat
			//qryTr182600();						// orderlist, open order value --> 800001
			//qryTr800000();			            // cash : rdi, arap, openbuy, cash, trd limit, cRat, pRat
		...
		function qryTr800011() {
			var userId = "<?=$clientId;?>";
			$.mobile.loading('show');
            // hichang, 20211006, no need get Ratio
			//tr800011(userId)
		...
		function tr800011(user) {
            pHashOrg = "tr=800011&userId=" + user + "&pin=" + "_S_pin"
		...
		function tr800005(clientID){
            pHashOrg = "tr=800005&clientID="+clientID;
            pHash    = "phash=" + Base64.encode(pHashOrg);
			$.ajax({
				 type: "POST",
				 url: "json/trproc.php",
		...
		function tr800000(user, client, date) {
			console.log("TESSSSSS : " + user + client + date);
			...
			$.ajax({
				...
				success: function(jsRet) {
					if (jsRet.status != 1) {
					    ...
					    console.log("jsRet 800000");
					    console.log(jsRet);	
						
						rows = [];
						rdi = sprintf("%d", Number(jsRet.out[0].rdn));
						arap = sprintf("%d", Number(jsRet.out[0].cashBalance) - Number(jsRet.out[0].rdn));
						openbuy = sprintf("%d", Number(jsRet.out[0].bid));
						cash = sprintf("%d", Number(jsRet.out[0].cashBalance) - Number(jsRet.out[0].bid));
						debt = Number(jsRet.out[0].cashBalance) - Number(jsRet.out[0].bid);
//						asset = g_totalStock + g_totalOpenBuyAmt;
						asset = Number(g_totalStock) + Number(g_totalOpenBuyAmt);
						trdlimit = sprintf("%d", (debt * 2.5) + (asset * 1.5));
						console.log("StkVal : " + g_totalStock + " OpBuyAmt : " + g_totalOpenBuyAmt);
						console.log("Asset = " + asset);
						console.log("RDI : " + Number(jsRet.out[0].cashBalance) + " OpBuy : " + Number(jsRet.out[0].bid));
						console.log("Cash = " + cash);
						console.log("Cash * 2.5 : " + Number(cash * 2.5) + " Asset * 1.5 : " + Number(asset * 1.5));
						...
						*/
						...
						g_totalNet = sprintf("%d", Number(String(g_totalNet).replace(',','')) +
												   Number(String(g_cash)));
						g_cell_v1 = [
							addCommas(g_totalBuyVal),
							addCommas(g_totalMktVal),
							addCommas(g_unrealized),
							addCommas(g_totalStock),
							addCommas(g_totalNet)
						];
						drawHeaderGrid('#r-headportfolio1', g_cell_v1);
					}
		...
		function tr182600(id, date) {
            pHashOrg = sprintf("tr=182600&userId=%s&cliId=%s&pin=&code=&board=&bsFlag=%s&status=%s&from=&to=&cnt=500&keyDate=&keyOrderNo=999999999&price=",id, id, "0", "0");
            pHash    = "phash=" + Base64.encode(pHashOrg);
			$.ajax({
				type: "POST",
				url: "json/trproc.php",
		...
		function tr800300(id, pin) {
            pHashOrg = "tr=800300&userId=" + id + "&pin=" + pin;
            pHash    = "phash=" + Base64.encode(pHashOrg);
			$.ajax({
				type: "POST",
				url: "json/trproc.php",
		...
		<br>
			<div class="bDiv" style="width:300px;">
				<table data-role="table" class="ui-responsive ui-shadow" id="" width ="100%">
					<thead>
						<tr>
							<th colspan="3" style="text-align:center;">Trade Due</th>
						</tr>
					</thead>
					<tbody>
					</tbody>
